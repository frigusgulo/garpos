...
def corr_attitude(rx, ry, rz, thy, thr, thp):
    ...
    yw = thy * math.pi / 180.0
    rl = thr * math.pi / 180.0
    pc = thp * math.pi / 180.0
    ...
    tr_rl = np.matrix([[1.0, 0.0, 0.0], [0.0, crl, -srl], [0.0, srl, crl]])

    tr_pc = np.matrix([[cpc, 0.0, spc], [0.0, 1.0, 0.0], [-spc, 0.0, cpc]])

    tr_yw = np.matrix([[cyw, -syw, 0.0], [syw, cyw, 0.0], [0.0, 0.0, 1.0]])
